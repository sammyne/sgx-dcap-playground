...
impl SGX {
    ...
    fn from_env() -> Result<Self, String> {
        ...
        let dcap_headers_path = env::var("DCAP_HEADERS_PATH").unwrap_or_else(|_| {
            let dir = fs::canonicalize("../../vendor/dcap").unwrap();
            dir.to_str().unwrap().to_string()
        });

        Self::new(&c_sdk_path, is_sim, &rust_sdk_path, dcap_headers_path)
    }
}
...
fn build_bridge_lib(sgx: &SGX, out_dir: &str) -> String {
    ...
    build
        ...
        .include(format!("{}", sgx.dcap_headers_path))
        .include(out_dir);
...
fn main() {
    ...
    println!("cargo:rustc-link-lib=dylib=dcap_quoteverify");
